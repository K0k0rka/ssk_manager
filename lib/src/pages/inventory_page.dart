...
class _InventoryPageState extends State<InventoryPage> {
  ...
  List<DataRow> getDataRow(List<Computer> computerList) {
    ...
    if (computerList.isEmpty) {
      ...
      for (var element in computerList) {
        if (tmpDropDownSelected.containsValue(element.model)) {
      ...
      for (var element in computerList) {
        if (tmpDropDownSelected.containsValue(element.model)){//contains(element.model))
          _computerSortList.add(element);
        }

      }
      return generateDataRow(_computerSortList);
    } else if (tmpDropDownSelected.isEmpty) {
      return generateDataRow(computerList);
    } else {
      return generateDataRow(computerList);
    }
  ...
  List<Computer> sortByStorage(List<Computer> dataSource){
    for(var i = 0; i < dataSource.length; i++) {
      if(dataSource[i].storage != tmpDropDownSelected['Storage']) {
        _computerSortList.remove(dataSource[i]);
      }
    }
    return _computerSortList;
  }

  List<Computer> sortByBuhNumber(List<Computer> dataSource){ //AK
    for(var i = 0; i < dataSource.length; i++) {
      if(dataSource[i].buhNumber != tmpDropDownSelected['Buh number']) {
        _computerSortList.remove(dataSource[i]);
      }
    }
    return _computerSortList;
  ...
  List<Computer> sortByModel(List<Computer> dataSource){ //AK
    for(var i = 0; i < dataSource.length; i++) {
      if(dataSource[i].model != tmpDropDownSelected['Model']) {
        _computerSortList.remove(dataSource[i]);
      }
    }
    return _computerSortList;
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              DropdownButton(
                                ...
                                value: tmpDropDownSelected['Model'],
                                ...
                                onChanged: (Object? value) {
                                  setState(() {
                                    tmpDropDownSelected['Model'] =
                                        value.toString();
                                    sortByModel(_computerSortList); // sortByModel(_computerList);
                                  });
                                },
                              ),
                            ],
                                ...
                                onChanged: (Object? value) {
                                  setState(() {
                                    tmpDropDownSelected['Buh number'] =
                                        value.toString();
                                    sortByBuhNumber(_computerSortList); //AK
                                  });
                                },
                              ),
                            ],
                                ...
                                onChanged: (Object? value) {
                                  setState(() {
                                    tmpDropDownSelected['Storage'] =
                                        value.toString();
                                    sortByStorage(_computerSortList);
                                  });
                                },
                              ),
                            ],
